...
% ----- MaxMin  Beampattern Matching Optimisation --------------%
function [R, Rk, mydelta,sidelobes_factor_dB_opt, status] = function_SDR_MaxMin_Optimisation_kappa(a, A_tgt, h, K, gamma, passband_idx, stopband_idx, P, Pt, M)
    ...
    epsillon = 0.05;            % fixed cross correlation
    ...
    cvx_begin sdp quiet
        ...
        maximize(mydelta-10*epsilon_stop)
    
        subject to
       % ----------------- Radar Constraints ------------%
       P_pass >= mydelta;
       P_stop <= epsilon_stop; %kappa*mydelta
